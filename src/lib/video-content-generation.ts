...
/**
 * Generate complete video content (title, description, question) for multiple subjects
 * This is optimized for YouTube Shorts content creation
 * 
 * @param className - The class name (e.g., "Class 11", "Class 12")
 * @param subjects - Array of subjects with their topics
 * @param topicsHistory - History of previously used topics to avoid repetition
 * @returns Array of complete video content for each subject
 */
...
`
    ...
    const prompt = `Create engaging YouTube Shorts quiz content for ${className} students across ${subjects.length} different subjects. Generate ONE question per subject optimized for YouTube Shorts.
...
You MUST respond with valid JSON only - no extra text!`
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are a creative educational content creator specializing in YouTube Shorts. Create engaging, curiosity-driven quiz content that makes learning fun and addictive. Focus on creating titles that make viewers want to click, descriptions that hook them immediately, and questions that spark genuine interest in the subject. Keep the question inspired by CBSE NEET/JEE/CBSE/ICSE syllabus. But do not include numerical problems, and heavy calculations as the content is for YouTube Shorts. There are enough theoretical problems asked in previous exams and books like Concepts of Physics, H.C. Verma, etc. It is very important that if more than one question is asked to be generated in one request, then provide enough variety of questions to cover all major aspects of the topic."
        },
        {
          role: "user",
          content: prompt

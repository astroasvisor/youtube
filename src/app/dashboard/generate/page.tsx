...
export default function GenerateVideoPage() {
  ...
  const [includeEducationalValue, setIncludeEducationalValue] = useState(false)
  const [includeHashtags, setIncludeHashtags] = useState(false)
  const [includeStudyTips, setIncludeStudyTips] = useState(false)
  ...
  const autoGenerateTitle = useCallback(() => {
    ...
    if (firstQuestion) {
      ...
      const subjectName = firstQuestion.topic.subject.name

      setVideoTitle(`${classSubject} | ${topic} Quiz #${Math.floor(Math.random() * 1000)}`)
      setVideoDescription(generateDetailedVideoDescription(
        count,
        topic,
        classSubject,
        subjectName,
        includeEducationalValue,
        includeHashtags,
        includeStudyTips
      ))
    }
  }, [selectedQuestions, questions, includeEducationalValue, includeHashtags, includeStudyTips])

  useEffect(() => {
    fetchApprovedQuestions()
  }, [])

  // Handle URL parameter and auto-generate title in one effect
  useEffect(() => {
    if (questionIdFromUrl && questions.length > 0 && !hasAutoGenerated.current) {
      setSelectedQuestions([questionIdFromUrl])
      
      // Auto-generate title after setting selected questions
      const timer = setTimeout(() => {
        const firstQuestion = questions.find(q => q.id === questionIdFromUrl)
        if (firstQuestion) {
          const classSubject = `${firstQuestion.topic.subject.class.name} ${firstQuestion.topic.subject.name}`
          const topic = firstQuestion.topic.name
  ...
  }, [questionIdFromUrl, questions, includeEducationalValue, includeHashtags, includeStudyTips])
          ...
          <div>
            ...
            <div className="space-y-2">
              <label className="flex items-center">
                <input
                  type="checkbox"
                  checked={includeEducationalValue}
                  onChange={(e) => setIncludeEducationalValue(e.target.checked)}
                  className="rounded"
                />
                <span className="ml-2 text-sm text-gray-700">Include educational value and career applications</span>
              </label>
              <label className="flex items-center">
                <input
                  type="checkbox"
                  ...
                  className="rounded"
                />
                <span className="ml-2 text-sm text-gray-700">Add relevant hashtags for better discoverability</span>
              </label>
              <label className="flex items-center">
                <input
                  type="checkbox"
                  checked={includeStudyTips}
                  onChange={(e) => setIncludeStudyTips(e.target.checked)}
                  className="rounded"
                />
                <span className="ml-2 text-sm text-gray-700">Include study tips and learning strategies</span>
              </label>
            </div>
          </div>
        </div>
